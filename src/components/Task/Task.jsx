...
function Task({
  ...
  return (
    <Fragment>
      {openModal && <ConfirmationModal isVisible={openModal} handleConfirm={() => deleteTask(task.id)}  handleCancel={() => setOpenModal(false)}/> }
      <Draggable
        key={task.id}
        draggableId={task.id}
        index={index}
      ...
      {(provided, snapshot) => {
        return (
          ...
          >
          <Card>
            <CardHeader title={task.title} className={classes.title} action={
              <IconButton aria-label="settings" onClick={() => setOpenModal(true)}>
                <DeleteIcon />
              </IconButton>
            }/>
